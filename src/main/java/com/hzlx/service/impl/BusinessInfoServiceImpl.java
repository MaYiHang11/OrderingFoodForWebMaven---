...
public class BusinessInfoServiceImpl implements BusinessInfoService {
    ...
    private OrderInfoDao orderInfo =new OrderInfoDaoImpl();
    @Override
...
//        String pwdSalt = MD5Utils.encryptMD5AndSalt(MD5Utils.encryptMD5(pwd), "吃了喵");
    ...
    public String goHome(HttpServletRequest request) {
        ...
        //计算预计收入
        BigDecimal total_money = orderInfo.getExpectedIncome(businessInfo.getId());
        //待处理订单列表
        List<Map<String, Object>> pendingOrders = orderInfo.getPendingOrderList(businessInfo.getId());
        //待处理订单数
        Long pendingNum = orderInfo.pending(businessInfo.getId());
        //往request作用域中存放数据
